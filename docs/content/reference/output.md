...
describes what users get. Pick a format with `-o`, or let speedrun choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
speedrun <command> -o table   # aligned columns for reading
speedrun <command> -o jsonl   # one JSON object per line, for piping
speedrun <command> -o json    # a single JSON array
speedrun <command> -o csv     # spreadsheet friendly
speedrun <command> -o tsv     # tab-separated
speedrun <command> -o url     # just the URL column
speedrun <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
speedrun <command> --fields id,title,url
```
...
```bash
speedrun <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
speedrun <command>            # a table, because this is a terminal
speedrun <command> | wc -l    # JSONL, because this is a pipe
```
